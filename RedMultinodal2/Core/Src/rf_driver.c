...
//Preset for Amplitud Shift Keying mod at 250KBits/s
uint8_t cc1100_AKS_115_kb[] = {
		0x0D,   // IOCFG2   -     GDO2 Output Pin Configuration //0x06 --> 0x02 (deasserts when below threshold)
		0x2E,  // IOCFG1        GDO1 Output Pin Configuration
		0x0D,   // IOCFG0   -     GDO0_Pin Output Pin Configuration
		0x47,  // FIFOTHR       RX FIFO and TX FIFO Thresholds //0x47 --> 0x48
...
uint8_t rf_begin(SPI_HandleTypeDef* hspi, MODULATION_TypeDef mode, ISMBAND_TypeDef ism_band, GPIO_TypeDef* cs_port, uint16_t cs_pin, uint16_t gdo0){
	 ...
	 * @param cs_port: Chip Select (SPI) Pin Port (ie: GPIOA)
	 * @param cs_pin: Chip Select (SPI) Pin number (ie: GPIO_Pin_4)
...
void test_cfg(){
		int i, distinto=0, a, b;
		for(i=0 ; i<47 ; i++){
					a=rf_read_register(0x00+i);
					b=cc1100_AKS_115_kb[i];
					HAL_Delay(10);
					if(a != b)
						distinto=1;
